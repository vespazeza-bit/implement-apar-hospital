  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <span style={{ marginLeft: 'auto', fontSize: 13, color: '#94a3b8' }}>แสดง {filtered.length} โครงการ</span>
      </div>
          ...
          <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 680, boxShadow: '0 24px 80px rgba(0,0,0,0.3)', marginBottom: 24 }}>
            <div style={{ background: 'linear-gradient(135deg, #1a2d4a, #1e3a5f)', padding: '20px 28px', borderRadius: '16px 16px 0 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <button type="button" onClick={() => setShowMasterPlan(true)} style={{ background: '#0891b2', border: 'none', borderRadius: 8, padding: '8px 16px', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                  🗓️ MasterPlan
                </button>
...
// ─── MasterPlan Modal ──────────────────────────────────────────────────────────
function MasterPlanModal({ onClose, plans, teamMembers, hospitals, onSaved }) {
  ...
  const loadItems = useCallback(async (planId) => {
    ...
    try { setItems(await api.get(`/masterplan-items?planId=${planId}`)) } catch { /**/ }
      ...
      for (const row of items) {
        if (String(row.id).startsWith('new_')) {
          const r = await api.post('/masterplan-items', {
            ...
            sortOrder: row.sortOrder, status: row.status || 'pending',
          })
          setItems(prev => prev.map(x => x.id === row.id ? { ...x, id: r.id, isNew: false } : x))
        } else if (editRows[row.id]) {
          await api.put(`/masterplan-items/${row.id}`, {
            ...
            preparation: row.preparation, status: row.status || 'pending',
          })
        }
      }
      setEditRows({})
  ...
  const hasUnsaved = items.some(r => String(r.id).startsWith('new_')) || Object.keys(editRows).length > 0

  const exportCSV = () => {
    const saved = items.filter(r => !String(r.id).startsWith('new_'))
      .sort((a, b) => {
        const da = (a.startDate || '9999') + (a.startTime || '')
        const db = (b.startDate || '9999') + (b.startTime || '')
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
                    ...
                    <tbody>
                      ...
                      <tr style={{ height: hoverInsert === 0 ? 28 : 4, transition: 'height 0.15s' }}
                        ...
                        <td colSpan={11} style={{ padding: 0, textAlign: 'center' }}>
                          {hoverInsert === 0 && (
                      ...
                      {items.map((row, idx) => (
                        <>
                          <tr key={row.id} style={{ background: String(row.id).startsWith('new_') ? '#f0fdf4' : '#fff' }}>
                  ...
                  {(() => {
                    ...
                    return (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
                        ...
                        <span style={{ fontSize: 12, color: '#64748b' }}>เรียงตามวันที่</span>
