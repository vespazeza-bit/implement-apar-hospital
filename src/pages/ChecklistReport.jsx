...
export default function ChecklistReport() {
  ...
  const loadEntries = useCallback(async (hospId) => {
    ...
    try {
      const data = await api.get(`/report-entries?hospitalId=${hospId}`)
  ...
  const loadImportRows = (hospId) => {
    api.get(`/report-entries?hospitalId=${hospId}`).then(existing => {
      const rows = reportMasterItems.map(m => {
  ...
  const sysOrder = reportMasterItems.map(m => m.systemName).filter((v, i, a) => v && a.indexOf(v) === i)
  ...
  const openEditMaster = (item) => {
    setMasterForm({ systemName: item.systemName, reportName: item.reportName, printName: item.printName, paperSize: item.paperSize, parameter: item.parameter, condition: item.condition || '', sortOrder: item.sortOrder })
    setEditId(item.id); setShowMasterForm(true)
  }
  const handleMasterSubmit = async (e) => {
    e.preventDefault()
    if (!masterForm.reportName.trim()) return alert('กรุณากรอกชื่อรายงาน')
    const payload = { ...masterForm, sortOrder: Number(masterForm.sortOrder) || 0 }
    if (editId) await updateReportMaster(editId, payload)
    else await addReportMaster({ ...payload, sortOrder: Number(masterForm.sortOrder) || reportMasterItems.length + 1 })
  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 22, color: '#1e3a5f', marginBottom: 4 }}>📈 Check List รายงาน</h2>
        <p style={{ color: '#64748b', fontSize: 13 }}>ตรวจสอบความพร้อมและความถูกต้องของรายงานต่างๆ ในระบบ AP/AR</p>
      </div>
      ...
      <div style={{ display: 'flex', marginBottom: 24, borderBottom: '2px solid #e2e8f0' }}>
        {[
          ...
          { key: 'master',    label: '⚙️ กำหนดข้อมูลรายงาน' },
        ].map(tab => (
      ...
      {/* ════ Tab: กำหนดข้อมูลรายงาน ════ */}
      {activeTab === 'master' && (
        <div>
          <div style={{ display: 'flex', gap: 14, marginBottom: 20, flexWrap: 'wrap' }}>
            <div style={{ background: '#eff6ff', borderRadius: 10, padding: '14px 20px', border: '1px solid #bfdbfe' }}>
              <div style={{ fontSize: 26, fontWeight: 700, color: '#1e3a5f' }}>{reportMasterItems.length}</div>
              <div style={{ fontSize: 12, color: '#64748b' }}>รายการรายงาน</div>
            </div>
                ...
                <tbody>
                  {filteredMaster.length === 0 ? (
                    ...
                    <tr key={item.id} style={{ background: idx % 2 === 0 ? '#fff' : '#fafafa' }}>
                      ...
                      <td style={{ ...tdS, fontWeight: 600, color: '#1e293b' }}>{item.reportName}</td>
                      ...
                      <td style={{ ...tdS, textAlign: 'center' }}>
                        <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
                          ...
                          <button onClick={() => handleMasterDelete(item.id, item.reportName)} style={{ padding: '4px 12px', background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca', borderRadius: 6, fontSize: 12, cursor: 'pointer' }}>ลบ</button>
                        </div>
                      </td>
                    </tr>
                  ))}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 400, padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 12, width: '100%', maxWidth: 820, boxShadow: '0 24px 80px rgba(0,0,0,0.3)', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '20px 28px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h3 style={{ color: '#1e3a5f', fontSize: 18, marginBottom: 2 }}>📈 เพิ่ม / นำเข้ารายการ Check List รายงาน</h3>
      ...
      {showMasterForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300, padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 28, width: '100%', maxWidth: 520, boxShadow: '0 20px 60px rgba(0,0,0,0.3)', maxHeight: '90vh', overflowY: 'auto' }}>
            <h3 style={{ marginBottom: 20, color: '#1e3a5f', fontSize: 18 }}>📈 {editId ? 'แก้ไข' : 'เพิ่ม'}รายการรายงาน</h3>
            <form onSubmit={handleMasterSubmit}>
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 5 }}>ชื่อรายงาน *</label>
                <input value={masterForm.reportName} onChange={setF('reportName')} placeholder="เช่น รายงานเจ้าหนี้คงเหลือ"
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <input value={masterForm.printName} onChange={setF('printName')} placeholder="เช่น AP Balance Report"
                  style={{ width: '100%', padding: '8px 12px', border: '1.5px solid #e2e8f0', borderRadius: 8, fontSize: 13, boxSizing: 'border-box' }} />
              </div>
              <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <textarea value={masterForm.condition} onChange={setF('condition')} rows={2} placeholder="ระบุเงื่อนไขการใช้งานรายงาน..."
                  style={{ width: '100%', padding: '8px 12px', border: '1.5px solid #e2e8f0', borderRadius: 8, fontSize: 13, resize: 'vertical', boxSizing: 'border-box' }} />
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
